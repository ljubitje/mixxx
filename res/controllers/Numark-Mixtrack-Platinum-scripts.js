...
MixtrackPlatinum.vuCallback = function(value, group, control) {
    ...
    // this logic shows master output's left and right vu meters in stereo when pfl is off
    // you can toggle vu meter of each deck on by toggling its respective pfl on
    if (pfl_left && pfl_right) {
        if (isChannelActive("left_deck")) {sendVuUpdate("left", group, level, "peak_indicator")}
        if (isChannelActive("right_deck")) {sendVuUpdate("right", group, level, "peak_indicator")}
    } else if (pfl_left) {
        if (isChannelActive("left_deck")) {sendVuUpdate("left", group, level, "peak_indicator")}
        if (isChannelActive("right_mix")) {sendVuUpdate("right", group, level, "peak_indicator_right")}
    } else if (pfl_right) {
        if (isChannelActive("left_mix")) {sendVuUpdate("left", group, level, "peak_indicator_left")}
        if (isChannelActive("right_deck")) {sendVuUpdate("right", group, level, "peak_indicator")}
    } else {
        if (isChannelActive("left_mix")) {sendVuUpdate("left", group, level, "peak_indicator_left")}
        if (isChannelActive("right_mix")) {sendVuUpdate("right", group, level, "peak_indicator_right")}
